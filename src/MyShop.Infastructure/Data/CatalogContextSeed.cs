    ...
    public class CatalogContextSeed
    {
        public static async Task SeedAsync(CatalogContext catalogContext, ILogger logger, int retry = 0)
        {
            ...
            try
            {
                ...
                {
                    await catalogContext.AddRangeAsync(GetPreConfigutedBrands());
                    await catalogContext.SaveChangesAsync();
                }

                if (!await catalogContext.CatalogTypes.AnyAsync())
                {
                    await catalogContext.AddRangeAsync(GetPreConfigutedCatalogTypes());
                    await catalogContext.SaveChangesAsync();
                }

                if(!await catalogContext.CatalogItems.AnyAsync())
                {
                    await catalogContext.AddRangeAsync(GetPreConfigutedCatalogItem());
                    await catalogContext.SaveChangesAsync();
                }
            }
            catch(Exception ex)
        ...
        static IEnumerable<CatalogBrand> GetPreConfigutedBrands()
        {
        ...
        static IEnumerable<CatalogType> GetPreConfigutedCatalogTypes()
        {
        ...
        static IEnumerable<CatalogItem> GetPreConfigutedCatalogItem()
        {
